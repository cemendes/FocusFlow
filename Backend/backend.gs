...
function pollAllEmails(targetName, daysBack) {
  ...
  const query = '-label:' + labelName + ' -category:promotions -category:social -category:updates -unsubscribe -is:draft newer_than:' + daysBack + 'd';
...
1. Project/Customer name (short, e.g., "Acme", "Internal").
